...
OUTPUT_PATH = WORK_DIR / "submission_19th_gpu_realmlp_6epoch_seed_ensemble.csv"
DIAGNOSTIC_DIR = WORK_DIR / "outputs" / "19th_gpu_realmlp_6epoch_seed_ensemble"
...
class RealMLPClassifier(BaseEstimator):
    ...
    def fit(
        ...
        self.epoch_records_ = epoch_records
        self.device_ = device
        return self
...
def main():
    ...
    if REQUIRE_CUDA and not torch.cuda.is_available():
        raise RuntimeError(
            ...
            "running the 19th RealMLP 6-epoch seed ensemble."
        )
    ...
    for seed_index, seed in enumerate(SEEDS, start=1):
        ...
        for fold, ((train_idx, valid_idx), (external_train_idx, _)) in enumerate(
            ...
            print("\n" + "=" * 60)
            print(f"Seed {seed} RealMLP Fold {fold}/{N_SPLITS}")
            print("=" * 60)
            ...
            model_config = {**CONFIG, "random_state": seed}
            model = RealMLPClassifier(**model_config)
            model.fit(
                X_train,
                y_train,
                X_valid,
                ...
                checkpoint_path=(
                    CHECKPOINT_DIR / f"model_seed_{seed}_fold_{fold}.pth"
                ),
            )
            ...
            epoch_records.extend(
                {
                    "seed": seed,
                    "fold": fold,
                    **record,
                }
                for record in model.epoch_records_
            )
            print(
                f"Seed {seed} RealMLP Fold {fold}: AUC={fold_auc:.6f}, "
    ...
    print("=" * 60)
    print(f"GPU RealMLP 6-epoch seed ensemble OOF AUC: {ensemble_auc:.6f}")
    print("\nSeed OOF AUC:")
    ...
    pd.DataFrame(epoch_records).to_csv(
        DIAGNOSTIC_DIR / "epoch_metrics.csv",
        index=False,
    )
    pd.DataFrame(
        {
            "metric": [
                "gpu_realmlp_6epoch_seed_ensemble_oof_auc",
