...
OUTPUT_PATH = DATA_DIR / "submission_lightgbm_fe.csv"
...
def add_row_features(df):
    ...
    df["LapNumberPerStint"] = safe_divide(lap_number, stint)
    df["LapsRemainingPerTyreLife"] = safe_divide(laps_remaining, tyre_life + 1.0)
    ...
    df["BackMarkerFlag"] = (position >= 15).astype(int)
...
def add_group_features(train, test):
    ...
    for group_cols, prefix in group_specs:
        grouped = all_features.groupby(group_cols, dropna=False)
        for col in stat_cols:
            mean_col = f"{prefix}_{col}_mean"
            std_col = f"{prefix}_{col}_std"
            train[mean_col] = grouped[col].transform("mean").iloc[: len(train)].to_numpy()
            test[mean_col] = grouped[col].transform("mean").iloc[len(train) :].to_numpy()
            train[std_col] = grouped[col].transform("std").iloc[: len(train)].to_numpy()
            test[std_col] = grouped[col].transform("std").iloc[len(train) :].to_numpy()

            train[f"{col}_diff_from_{prefix}_mean"] = train[col] - train[mean_col]
            test[f"{col}_diff_from_{prefix}_mean"] = test[col] - test[mean_col]

    return train.replace([np.inf, -np.inf], np.nan), test.replace([np.inf, -np.inf], np.nan)


def build_features(train, test):
    train_fe, test_fe = add_group_features(train, test)
...
def main():
    ...
    print(f"Features after FE: {len(X.columns)}")
